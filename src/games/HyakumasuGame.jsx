...
export default function HyakumasuGame({ lang, setLang, onExit }) {
  ...
  return (
    <div className={`min-h-screen bg-slate-900 text-white flex flex-col items-center select-none transition-colors ${wrongFlash ? 'bg-red-500/10' : ''}`}>
      ...
      <header className="w-full max-w-[430px] flex items-center justify-between px-4 py-3 border-b border-slate-700 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-orange-400 font-black text-base">{t('百マス計算', 'Hundred Squares')}</span>
        </div>
        <button onClick={() => setLang(l => (l === 'ja' ? 'en' : 'ja'))}
          className="text-xs border border-slate-600 rounded px-2 py-1 text-slate-300 hover:border-orange-400 hover:text-orange-400 transition">
          {lang === 'ja' ? 'EN' : 'JP'}
        </button>
      </header>
        ...
        {screen === 'play' && ready && (
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2">
                {versus && (
                  <span className={`px-2 py-1 rounded-lg font-black ${player === 1 ? 'bg-sky-500 text-white' : 'bg-pink-500 text-white'}`}>
                    {t('プレイヤー', 'Player')}{player}
                  </span>
                )}
                <span className="text-slate-400">{t('ミス', 'Miss')} <b className="text-red-300">{mistakes}</b></span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-2xl font-black text-orange-300 tabular-nums">⏱ {fmtTime(elapsed)}</span>
              </div>
            </div>
            ...
            {/* 大きな出題カード */}
            <div className="bg-slate-800 border-2 border-orange-500/60 rounded-2xl py-5 text-center shadow-lg shadow-orange-500/10">
              ...
              <div className="text-xs text-slate-500 mt-1">{index + 1} / {CELLS} マス目</div>
            </div>

            {/* 百マス盤面（いま解いているマスと見出しが光る） */}
...
function HyakumasuGrid({ rowH, colH, answers, index, sym }) {
  ...
  return (
    <div className="grid gap-[2px] mx-auto w-full" style={{ gridTemplateColumns: `repeat(${SIZE + 1}, minmax(0, 1fr))` }}>
      {/* 左上の角（演算記号） */}
      <div className="aspect-square flex items-center justify-center rounded-[3px] bg-slate-600 text-white font-black text-xs">{sym}</div>
      {/* 上端の列見出し */}
      {colH.map((c, ci) => (
        <div key={`c${ci}`}
          className={`aspect-square flex items-center justify-center rounded-[3px] font-black text-[11px] transition-colors
            ${ci === curC ? 'bg-yellow-400 text-slate-900' : 'bg-slate-700 text-slate-200'}`}>
          {c}
        </div>
      ))}
      {/* 各行 */}
      {rowH.map((rv, ri) => (
        <Fragment key={`r${ri}`}>
          ...
          <div className={`aspect-square flex items-center justify-center rounded-[3px] font-black text-[11px] transition-colors
            ${ri === curR ? 'bg-yellow-400 text-slate-900' : 'bg-slate-700 text-slate-200'}`}>
            {rv}
          </div>
          {/* マス */}
          {colH.map((_, ci) => {
            ...
            return (
              <div key={idx}
                className={`aspect-square flex items-center justify-center rounded-[3px] text-[10px] font-bold transition-colors
                  ${isCur
                    ? 'bg-orange-400 text-slate-900 ring-2 ring-yellow-200 animate-pulse'
                    : solved
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="text-center">
        <div className="text-3xl font-black text-orange-400 mb-1">{t('百マス計算', 'Hundred Squares')}</div>
      ...
      <div>
          ...
          {OP_ORDER.map(o => (
            <button key={o} onClick={() => setOp(o)}
              className={`py-4 rounded-2xl font-black text-lg border-2 transition active:scale-95 cursor-pointer
                ${op === o ? 'bg-orange-500 border-orange-300 text-white' : 'bg-slate-800 border-slate-700 text-slate-300 hover:border-orange-400'}`}>
        ...
        {op === 'sub' && (
          <div className="text-[11px] text-slate-500 mt-2">{t('※ ひき算は答えがマイナスにならないように出題します', '* Subtraction never goes below zero')}</div>
        )}
      </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => setVersus(false)}
            className={`py-4 rounded-2xl font-black border-2 transition active:scale-95 cursor-pointer
              ${!versus ? 'bg-sky-600 border-sky-300 text-white' : 'bg-slate-800 border-slate-700 text-slate-300'}`}>
            <div className="text-2xl">🧒</div>
            <div className="text-sm mt-1">{t('ひとりで', 'Solo')}</div>
          </button>
          <button onClick={() => setVersus(true)}
            className={`py-4 rounded-2xl font-black border-2 transition active:scale-95 cursor-pointer
              ${versus ? 'bg-pink-600 border-pink-300 text-white' : 'bg-slate-800 border-slate-700 text-slate-300'}`}>
            <div className="text-2xl">👨‍👧</div>
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          {OP_ORDER.map(o => (
            <div key={o}>
              ...
              <div className={`text-sm font-black tabular-nums ${best[o] != null ? 'text-orange-300' : 'text-slate-600'}`}>
      ...
      <button onClick={onStart}
        className="w-full bg-orange-500 text-white font-black text-xl py-4 rounded-2xl hover:bg-orange-400 active:scale-95 transition cursor-pointer">
        {t('スタート', 'Start')} ▶
      </button>
    </div>
  )
...
function SwitchScreen({ t, result, onNext }) {
  return (
    <div className="flex flex-col items-center justify-center gap-6 py-10 text-center">
      <div className="text-2xl font-black text-sky-300">{t('プレイヤー1 おわり！', 'Player 1 done!')}</div>
      <div className="bg-slate-800 rounded-2xl px-8 py-5 border border-slate-700">
        <div className="text-4xl font-black text-orange-300 tabular-nums">⏱ {fmtTime(result.timeMs)}</div>
        <div className="text-slate-400 mt-1">{t('ミス', 'Miss')} {result.mistakes}</div>
      </div>
      <div className="text-lg font-bold text-pink-300">👉 {t('プレイヤー2 にこうたい！', 'Player 2\'s turn!')}</div>
      <button onClick={onNext}
        className="w-full bg-pink-500 text-white font-black text-xl py-4 rounded-2xl hover:bg-pink-400 active:scale-95 transition cursor-pointer">
...
function ResultScreen({ t, op, versus, results, isRecord, best, onRetry, onConfig }) {
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-4">
      <div className="text-2xl font-black text-orange-400">{OPS[op].label[t('ja', 'en')] || OPS[op].label.ja} {t('かんりょう！', 'Complete!')}</div>

      {!versus && results[0] && (
        <div className="flex flex-col items-center gap-2">
          <div className="text-6xl font-black text-orange-300 tabular-nums">{fmtTime(results[0].timeMs)}</div>
          <div className="text-slate-400">{t('ミス', 'Miss')} {results[0].mistakes}</div>
          {isRecord && <div className="text-yellow-300 font-black text-lg animate-bounce">🎉 {t('ベスト記録こうしん！', 'New Best!')}</div>}
          {!isRecord && best[op] != null && (
            <div className="text-xs text-slate-500">{t('ベスト', 'Best')}: {fmtTime(best[op])}</div>
          )}
        </div>
      )}
            ...
            return (
              <div key={i}
                className={`flex items-center justify-between rounded-2xl px-5 py-4 border-2
                  ${isWin ? 'bg-yellow-400/15 border-yellow-400' : 'bg-slate-800 border-slate-700'}`}>
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-1 rounded-lg font-black text-sm ${r.player === 1 ? 'bg-sky-500' : 'bg-pink-500'} text-white`}>
                    {t('プレイヤー', 'P')}{r.player}
                  </span>
                  {isWin && <span className="text-xl">👑</span>}
                </div>
                <div className="text-right">
                  <div className="text-2xl font-black text-orange-300 tabular-nums">{fmtTime(r.timeMs)}</div>
                  <div className="text-xs text-slate-400">{t('ミス', 'Miss')} {r.mistakes}</div>
                </div>
              </div>
            )
          })}
          <div className="text-center text-xl font-black text-yellow-300 mt-1">
      ...
      <div className="flex flex-col gap-3 w-full mt-2">
        <button onClick={onRetry}
          className="w-full bg-orange-500 text-white font-black text-lg py-4 rounded-2xl hover:bg-orange-400 active:scale-95 transition cursor-pointer">
          {t('もう一回', 'Again')}
        </button>
